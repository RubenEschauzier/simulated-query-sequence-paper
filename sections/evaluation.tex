...
\rt{Important to explain clearly: what is the scope of the cache? At what points is the cache flushed? How does it relate to repeating sequences?}
...
Secondarily, we establish a caching performance baseline for a single-threaded \rt{Not sure if "single-threaded" is needed here. It might raise more questions than it would do good.} LTQP engine. 
...
increasing speed and solving \emph{more} queries than the baseline. 
Notably, larger cache sizes degrade its performance. 
Indicating that cardinality estimation favors smaller, fresher caches, 
as it evaluates all entries rather than only traversal-relevant ones.\rt{This is surprising. I would expect larger caches to provide more accuracy for cardinality estimation. Not sure what to do with this surprising fact besides just reporting though. Maybe you can double-check if it's really correct?}
...
\rt{Or: separate caches per sessions need to be kept (which requires engines to identify queries being within a session)}

\begin{table}[htpb]
\centering
\caption{
    Impact of logical session status on algorithm performance. Values represent absolute and percentage cache hit rate deviations from the global template average across new, existing, and within-session queries. \rt{Mention clearly if negative values are good or bad.}
}
...
\begin{figure}[htpb]
    ...
    \caption{Cache hit ratios and percentage of cache capacity evicted across refinement depths for small, medium, and large cache configurations.\rt{I would recommend using "Cache hit rate (\%)" on the Y axis}}
...
schedules asynchronous traversal and join executions \rt{In order to understand this sentence, we definitely need this high-level description of LTQP, and how it's implemented in Comunica.}. 
This scheduling variance alters traversal order and rate, 
ultimately impacting cache eviction and hit performance.\rt{I understand the variance, but from this text, I don't understand why this leads to higher hit rate with smaller capacities.}
...
The observed cache hitrate \rt{It's "hit rate", not "hitrate"} and eviction differences directly translate to measurable execution speedups.
Figure 2 illustrates that the geometric mean speedup for mainly \rt{The "mainly" feels wrong here} throughput generally scales positively with refinement depth. 
...
yielding initial results faster since only the first few requests are needed.\rt{This last sentence is clear to me, but is another example that requires more background in a dedicated Comunica LTQP description}
...
\begin{figure*}[!h]
    ...
    The y-axis shows the mean speedup relative to a baseline with a 0\% hit rate.
     Shaded regions denote the 90th percentile \rt{Why is the third caching approach not mentioned? If irrelevant, make sure to explain why.}}
