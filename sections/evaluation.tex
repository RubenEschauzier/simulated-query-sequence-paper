...
Secondarily, we establish a caching performance baseline for a single-threaded LTQP engine. 
...
For the unindexed cache, small (350,000 triples) and medium (700,000 triples) sizes 
perform similarly, while the large cache (1,400,000 triples) significantly improves speed 
by reaching the critical capacity required to retain entries across the sequence. 
However, the baseline still \emph{solves} more total queries, suggesting that cache 
management overhead causes some long-running queries to time out.

The indexed cache shows an even higher failure rate, confirming that initial 
indexing costs slow execution during frequent evictions. 
Yet, for successful queries, the indexed cache yields a larger performance boost than unindexed ones. 
The indexed cache also demonstrates a lower critical capacity, 
as medium and large sizes perform similarly.

Ultimately, the indexed cache with cardinality estimation performs best, 
improving execution speeds and solving \emph{more} total queries than the baseline. 
Notably, increasing the cache size degrades performance for this method, 
with the large cache underperforming the smaller versions. 
This implies that cardinality estimation benefits from a smaller, less stale cache, 
as the estimation process evaluates all cache entries rather than just traversal-relevant ones.
...
To analyze theoretical caching speedup without management costs, 
we executed each query template twice with simulated cache miss probabilities. 
