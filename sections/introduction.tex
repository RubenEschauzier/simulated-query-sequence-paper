...
\begin{todolist}
    ...
    \item Same but for optimized join plan computed prior to execution. (should be just a config parameter in experiment)
    ...
    \item (If caching still performs poorly) x-axis: simulated cache-hit rate, y-axis query execution time (for single template). For bad join plan / good join plan.
    ...
    \item Hitrate after session switch (switch new, switch old)
    \item Hitrate after session switch (without probability based switching) 

    \item Within refinement sequence hitrate
    \item Cache performance within refinement sequence compared to default (hitrate, evictions) show average per step (so first refine step second etc) (use cumulative)
...
\begin{todolist}
    ...
    \item Disk-based cache
    \item Adaptive join + caching of documents
...
or relevance indexes to efficiently execute LTQP queries~\cite{tam2024opportunities,umbrich2011comparing,hanski2024observations}.
